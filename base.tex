...
\begin{frame}
  ...
  \begin{lem}[\texttt{And.intro}]
    If the propositions 
    \only<1,3>{$A$ and $B$}
    \only<2>{\setulcolor{orange}\ul{$A$} and \ul{$B$}}
    are true and so is
    \only<1,2>{$A \land B$}
    \only<3>{\setulcolor{blue}\ul{$A \land B$}.}
  \end{lem}
\end{frame}
...
\begin{frame}
  ...
  \begin{lem}[\texttt{le\_trans}]
    ...
    \only<2>{$$a \leq b \rightarrow b \leq c \rightarrow \mul{orange}{a \leq c}.$$}
    \only<3>{$$\mul{blue}{a \leq {\color{blue}{b}} \rightarrow {\color{blue}{b}} \leq c} \rightarrow a \leq c.$$}
  \end{lem}
...
\begin{frame}
  ...
  \blue{Iteratively fill the matches}: find a hypothesis for \blue{slot 1}, then find a hypothesis for \blue{slot 2} that \blue{agrees on the shared variables}, \dots

  When \blue{adding} a hypothesis, \blue{extend the matches} that agree with this hypothesis.
