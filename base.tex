...
\begin{frame}
  ...
  \begin{itemize}
    \item Forward : 
    From the premises to the conclusion of the rule.
    \begin{center}
      Work on the goal's hypotheses forwards to the target.
    \end{center}
    
    \item Backward : 
    From the conclusion to the premises.
    \begin{center}
      Work on the goal's target backwards to the hypotheses.
    \end{center}
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Forward and Backward Reasoning in Lean}
  \begin{itemize}
    \item Forward Reasoning : \lean{have} and \lean{obtain}
    \item Backward Reasoning : \lean{apply}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{lem}[\lean{And.intro}]
    If the propositions 
    \only<1>{$A$ and $B$}
    \only<2>{\setulcolor{red}\ul{$A$} and \ul{$B$}}
    are true and so is
    \only<1>{$A \land B$}
    \only<2>{\setulcolor{teal}\ul{$A \land B$}}.
  \end{lem}
\end{frame}
...
\begin{frame}
  ...
  \begin{lem}[\lean{le_trans}]
    ...
    \only<2>{$$a \leq b \rightarrow b \leq c \rightarrow \mul{red}{a \leq c}.$$}
    \only<3>{$$\mul{teal}{a \leq {\color{teal}{b}} \rightarrow {\color{teal}{b}} \leq c} \rightarrow a \leq c.$$}
  \end{lem}
...
\begin{frame}
  \frametitle{What is a Good Forward Rule?}
  \begin{lem}[\lean{cauchySeq_tendsto_of_complete}]
    \setulcolor{teal}
    \only<1>{Let $(a_n)$ be a Cauchy sequence in a complete space $X$.
    
    The sequence $(a_n)$ converges.
    }
    \only<3>{Let $(a_n)$ be a \ul{Cauchy} sequence in a \ul{complete} space $X$.
...
\begin{frame}
  \frametitle{What is a Good Forward Rule?}
  \begin{lem}[\lean{IsClosed.isSeqClosed}]
    \setulcolor{teal}
    \only<1>{
...
\begin{frame}
  \frametitle{Disclaimer : Unsafe Backward Rule}
  Backward rules try "harder":
  \begin{itemize}
    \item<2-> Forward rules {\color{red}{will not}} succeed if one of the premises is missing from the context.
    \item<3-> Backward rules {\color{blue}{might}} succeed as it will also try to prove the premises.
  \end{itemize}
\end{frame}
